...
function handleFormSubmit() {
   ...
   .map(row => columnIndices.map(index => row[index]));
        ...
        fetch('save_data_jateng.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ keterangan: keterangan, fileData: filteredData })
...
function viewDetails(id) {
    fetch(`get_details_jateng.php?id=${id}`)
        .then(response => response.json())
        .then(data => {
            ...
            // Filter data untuk hanya menampilkan kode Plant yang diinginkan
            const filteredData = data.filter((row, index) => {
                if (index === 0) return true; // Baris header tetap dipertahankan

                const plantCode = row[plantColumnIndex];
                const shouldKeep = ['F343'].includes(plantCode); // Filter hanya untuk kode ini

                console.log(`Baris ${index}:`, {
                    plantCode: plantCode,
                    dipertahankan: shouldKeep
                });
            ...
            // Tampilkan data yang telah difilter di halaman detail
